...
def train(model, train_loader, val_loader, criterion, optimizer, Epochs, test_loader, model_name="Model"):
    ...
    for epoch in range(Epochs):
        ...
        avg_val_loss, val_acc = calculate_accuracy(val_loader, model)
...
all_results = []
configs = list(itertools.product(lr, optimizers, batch_sizes, regularizations))
for lr_val, opt_name, batch_size, reg in configs:
    ...
    if opt_name == 'SGD':
        optimizer = optim.SGD(model.parameters(), lr=lr_val, weight_decay=reg, momentum=0.9)
    elif opt_name == 'Adam':
        optimizer = optim.Adam(model.parameters(), lr=lr_val, weight_decay=reg)
    elif opt_name == 'RMSprop':
        optimizer = optim.RMSprop(model.parameters(), lr=lr_val, weight_decay=reg)
    Epochs = 10
    file_name = f"LogReg_LR{lr_val}_Opt{opt_name}_BS{batch_size}_Reg{reg}"
...
layer_sizes = [256, 512]
all_results = []
configs = list(itertools.product(lr, optimizers, batch_sizes, regularizations, layer_sizes))
top_acc = 0;
for lr_val, opt_name, batch_size, reg, layer_size in configs:
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False)
    model = nn.Sequential(
